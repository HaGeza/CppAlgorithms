...
std::pair<vec_i, int> DFS(int node, int sink, vec<vec<pair_ii>> &flowGraph,
    ...
    for (auto edge : flowGraph[node]) {
        ...
        if (seen[next] || weight == 0) continue;

        auto pathAndFlow =
            DFS(next, sink, flowGraph, std::min(currentFlow, weight), seen);
...
int fordFulkerson(int source, int sink, vec<vec<pair_ii>> graph) {
    auto flowGraph = vec<vec<pair_ii>>(graph);
    int totalFlow = 0;
